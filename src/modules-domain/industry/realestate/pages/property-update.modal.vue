      ...
      <div class="space-y-4">
        ...
        <div class="grid grid-cols-2 gap-4">
          ...
          <u-form-field label="상태">
            <u-select v-model="formData.status" :items="statusItems" class="w-full" />
          </u-form-field>
        </div>
...
import type { PropertyUpdateDto } from '../type/realestate.type';
...
const formData = ref<PropertyUpdateDto>({
  propertySeq: 0,
  propertyName: '',
  propertyType: '아파트',
  transactionType: '매매',
...
const register = async () => {
  ...
  await FormService.loading(async () => {
    await realestateStore.propertyUpdate(formData.value);
    emit('update-ok');
    close();
  });
};
...
const getDetail = async () => {
  await FormService.loading(async () => {
    const detail = await realestateStore.propertyDetail(props.propertySeq);
    if (detail && detail.propertySeq) {
      formData.value = {
        propertySeq: detail.propertySeq,

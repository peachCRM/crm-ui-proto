<template>
  <form @submit.prevent="listAction">
    <div class="w-full rounded-lg bg-white">
      ...
      <div v-if="isSearchExpanded" class="overflow-hidden">
        <div class="mt-3 flex flex-col gap-3 p-6 pt-0">
          <div class="flex w-full flex-col items-center gap-3 lg:flex-row">
            ...
            <p-input-box v-model="listParams.keyword" placeholder="거래처를 입력하세요." />
          </div>
          <div class="flex w-full flex-col items-center gap-3 lg:flex-row">
            <div class="w-20 shrink-0 font-semibold text-gray-600">사용여부</div>
            <div class="flex gap-4">
              <p-radiobox
                v-for="item in optIsUseList"
                :key="item.value"
      ...
      >
        <p-button color="btn_outLine_default_sm" @click="resetAction"> 초기화 </p-button>
        <div class="flex items-center gap-2">
          <p-button color="btn_primary_md" type="submit"> 상세조건 검색 </p-button>
          <p-button
            color="btn_outLine_default_sm"
            @click="toggleSearch"
...
import PButton from '@/modules/_common/components/forms/p-button.vue';

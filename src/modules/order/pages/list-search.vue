    ...
    <div class="w-full rounded-lg bg-white">
      <div class="space-y-3 p-6" :class="{ 'pb-0': isSearchExpanded }">
        <div class="flex w-full flex-col items-center gap-3 lg:flex-row">
          <div class="w-20 shrink-0 font-semibold text-gray-600">주문일</div>
          ...
          <div class="w-20 shrink-0 text-center font-semibold text-gray-600">검색</div>
      ...
      <div v-if="isSearchExpanded" class="overflow-hidden">
        <div class="mt-3 flex flex-col gap-3 p-6 pt-0">
          <div class="flex w-full flex-col items-center gap-3 lg:flex-row">
            <div class="w-20 shrink-0 font-semibold text-gray-600">주문상태</div>
            <p-nuxt-select
              ...
              @change="listAction"
            />
            <div class="w-20 shrink-0 text-center font-semibold text-gray-600">결제수단</div>
            <p-nuxt-select
              v-model="listParams.paymentMethod"
...
// 검색 파라미터 로컬 상태로 관리
const listParams = ref<OrderSearchDto>({
...
watch(
  ...
  () => {
    if (route.query && Object.keys(route.query).length > 0) {
      ...
      const { startDate, endDate, keyword, opt, orderStatus, paymentMethod } = route.query;
      Object.assign(listParams.value, { startDate, endDate, keyword, opt, orderStatus, paymentMethod });
    } else {
      // 검색 파라미터 초기화
      resetAction();
    }

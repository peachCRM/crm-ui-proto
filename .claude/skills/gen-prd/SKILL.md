...
| 용도 | PostgreSQL | MySQL |
|------|-----------|-------|
| PK | serial4 | BIGINT AUTO_INCREMENT |
| 정수 | int4 | INT |
| 큰 정수 | int8 | BIGINT |
...
**후속 스킬:** 구현 (How)
- **gen-db**: 스키마 → SQL 마이그레이션 파일
- **gen-backend**: PRD → Backend API 코드
- **gen-store**: PRD → Frontend Store 코드
- **gen-ui**: PRD → UI 컴포넌트 코드
...
- `PK_TYPE` → serial4 (PostgreSQL) / BIGINT (MySQL)
- `PK_DEFAULT` → 자동증가 (PostgreSQL) / AUTO_INCREMENT (MySQL)
- `FK_TYPE` → int4 (PostgreSQL) / BIGINT (MySQL)
...
**다음 단계:**
- `/gen-db` - DB 스키마 → 마이그레이션 파일 생성
- `/gen-backend` - Backend API 코드 생성
- `/gen-store` - Frontend Store 코드 생성
- `/gen-ui` - UI 컴포넌트 코드 생성
```
